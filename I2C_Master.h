...
unsigned char I2C_LEITURA (unsigned char ADRR, unsigned char addr)	//função I2C_LEITURA
{
	char x;								//declaração de variável local 
	...
	else 								//barramento livre
	{
		...
		I2C_TRANSMITE(ADRR);			//transmite endereço
		if (!I2C_TESTA_ACK()) 			//se erro na transmissão, aborta transmissão
		{
			I2C_STOP();					//gera bit STOP
			return -1;					//erro na transmissão
 		}
        I2C_TRANSMITE(addr);			//transmite endereço
		if (!I2C_TESTA_ACK()) 			//se erro na transmissão, aborta transmissão
		{
			I2C_STOP();					//gera bit STOP
			return -1;					//erro na transmissão
        ...
        I2C_RESTART();
        //com_START();
        I2C_TRANSMITE(ADRR | 0x01);	//transmite endereço
		if (!I2C_TESTA_ACK())                   //se erro na transmissão, aborta transmissão
		{
			I2C_STOP();					//gera bit STOP
			return -1;					//erro na transmissão
